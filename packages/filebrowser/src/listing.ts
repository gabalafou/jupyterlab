...
export class DirListing extends Widget {
  ...
  async delete(): Promise<void> {
    ...
    if (focusIndex > this._sortedItems.length - items.length - 1) {
      focusIndex = Math.max(0, this._focusIndex - 1);
    }
  ...
  protected onUpdateRequest(msg: Message): void {
    ...
    nodes.forEach((node, i) => {
      ...
      if (nameNode) {
        // Gotta check if the name node is there because it gets replaced by the
  ...
  private _onPathChanged(): void {
    ...
    // Reset focus.
    //
    // Note: this code may be slightly misleading, it may suggest that the focus
    // method is being called on the first item after the directory listing has
    // refreshed. But actually, because the DOM update gets queued via
    // `this.update()`, the focus method is called before the items refresh. The
    // only reason this works is because of an implementation detail of this
    // class: it reuses DOM nodes where it can, rather than destroying and
    // recreating them.
    this._focusItem(0);
  }
